...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///milestone.db'
db = SQLAlchemy(app)
...
@app.route('/api/milestone/<int:id>', methods=['GET'])
def get_milestone(id):
    item = Milestone.query.get_or_404(id)
    return jsonify(item.to_dict())

@app.route('/api/milestone', methods=['POST'])
def create_milestone():
    data = request.get_json()
    item = Milestone(name=data.get('name'))
    db.session.add(item)
    db.session.commit()
