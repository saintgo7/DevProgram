...
def get_all_checkout():
    items = Checkout.query.all()
    return jsonify([item.to_dict() for item in items])

@app.route('/api/checkout/<int:id>', methods=['GET'])
def get_checkout(id):
    item = Checkout.query.get_or_404(id)
    return jsonify(item.to_dict())

@app.route('/api/checkout', methods=['POST'])
def create_checkout():
    data = request.get_json()
    item = Checkout(name=data.get('name'))
    db.session.add(item)
    db.session.commit()
...
@app.route('/api/checkout/<int:id>', methods=['DELETE'])
