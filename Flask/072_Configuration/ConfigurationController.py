...
@app.route('/api/configuration/<int:id>', methods=['GET'])
...
def create_configuration():
    ...
    item = Configuration(name=data.get('name'))
    db.session.add(item)
    db.session.commit()
    return jsonify(item.to_dict()), 201

@app.route('/api/configuration/<int:id>', methods=['PUT'])
...
@app.route('/api/configuration/<int:id>', methods=['DELETE'])
