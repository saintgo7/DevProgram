...
@app.route('/api/manufacturer/<int:id>', methods=['GET'])
...
def create_manufacturer():
    ...
    item = Manufacturer(name=data.get('name'))
    db.session.add(item)
    db.session.commit()
    return jsonify(item.to_dict()), 201

@app.route('/api/manufacturer/<int:id>', methods=['PUT'])
...
@app.route('/api/manufacturer/<int:id>', methods=['DELETE'])
