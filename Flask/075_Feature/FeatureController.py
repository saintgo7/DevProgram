...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///feature.db'
db = SQLAlchemy(app)
...
@app.route('/api/feature/<int:id>', methods=['GET'])
def get_feature(id):
    item = Feature.query.get_or_404(id)
    return jsonify(item.to_dict())

@app.route('/api/feature', methods=['POST'])
def create_feature():
    data = request.get_json()
    item = Feature(name=data.get('name'))
    db.session.add(item)
    db.session.commit()
