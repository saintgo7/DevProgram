...
@app.route('/api/delivery/<int:id>', methods=['GET'])
def get_delivery(id):
...
def create_delivery():
    ...
    item = Delivery(name=data.get('name'))
    db.session.add(item)
    db.session.commit()
    return jsonify(item.to_dict()), 201

@app.route('/api/delivery/<int:id>', methods=['PUT'])
...
@app.route('/api/delivery/<int:id>', methods=['DELETE'])
