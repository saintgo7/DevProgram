...
def get_shipment(id):
    item = Shipment.query.get_or_404(id)
    return jsonify(item.to_dict())

@app.route('/api/shipment', methods=['POST'])
def create_shipment():
    data = request.get_json()
    item = Shipment(name=data.get('name'))
    db.session.add(item)
    db.session.commit()
    return jsonify(item.to_dict()), 201

@app.route('/api/shipment/<int:id>', methods=['PUT'])
...
@app.route('/api/shipment/<int:id>', methods=['DELETE'])
