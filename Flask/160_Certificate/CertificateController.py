...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///certificate.db'
db = SQLAlchemy(app)
...
@app.route('/api/certificate/<int:id>', methods=['GET'])
...
def create_certificate():
    ...
    item = Certificate(name=data.get('name'))
    db.session.add(item)
    db.session.commit()
    return jsonify(item.to_dict()), 201

@app.route('/api/certificate/<int:id>', methods=['PUT'])
